...
class MarkerApp(ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        self.content_navigation_frame=ctk.CTkFrame(self,fg_color=("gray90","gray18"),corner_radius=8); self.content_navigation_frame.grid(row=1,column=0,padx=20,pady=(8,0),sticky="ew"); self.content_navigation_frame.grid_columnconfigure(0,weight=2); self.content_navigation_frame.grid_columnconfigure(2,weight=3)
        group_font=ctk.CTkFont(size=13,weight="bold")
        self.media_group_label=ctk.CTkLabel(self.content_navigation_frame,text="",text_color=("gray35","gray75"),font=group_font,height=18); self.media_group_label.grid(row=0,column=0,padx=10,pady=(3,0),sticky="w")
        self.media_navigation=ctk.CTkSegmentedButton(self.content_navigation_frame,variable=self.content_navigation_var,values=["Images","Video"],command=self.change_content_workspace,height=28); self.media_navigation.grid(row=1,column=0,padx=10,pady=(0,5),sticky="ew")
        ctk.CTkFrame(self.content_navigation_frame,width=1,fg_color=("gray72","gray35")).grid(row=0,column=1,rowspan=2,padx=5,pady=5,sticky="ns")
        self.documents_group_label=ctk.CTkLabel(self.content_navigation_frame,text="",text_color=("gray35","gray75"),font=group_font,height=18); self.documents_group_label.grid(row=0,column=2,padx=10,pady=(3,0),sticky="w")
        self.document_navigation=ctk.CTkSegmentedButton(self.content_navigation_frame,variable=self.content_navigation_var,values=["PDF","PowerPoint / Slides","Word"],command=self.change_content_workspace,height=28); self.document_navigation.grid(row=1,column=2,padx=10,pady=(0,5),sticky="ew")
    ...
    def _document_ui(self) -> None:
        ...
        self.pptx_selected_label=ctk.CTkLabel(self.pptx_selected_frame,text="",anchor="w"); self.pptx_selected_label.grid(row=0,column=0,sticky="w"); ctk.CTkEntry(self.pptx_selected_frame,textvariable=self.pptx_selected_var).grid(row=1,column=0,sticky="ew")
    ...
    def choose_pdf(self):
        ...
        self.pdf_path=Path(selected); self.pdf_preview_renderer=PdfPreviewRenderer(self.processor); self.pptx_slide_number=1; self._pdf_warning_approved=None; self._pdf_signature_approved=None
    ...
    def change_pdf_preview_page(self,delta):
        if self.pptx_slide_count:self.pptx_slide_number=min(self.pptx_slide_count,max(1,self.pptx_slide_number+delta)); self.update_pdf_preview()
    ...
    def _clear_pptx_selection(self):
        self.pptx_path=None; self.pptx_metrics=None; self._pptx_warning_approved=None; self.pptx_slide_count=0; self.pptx_preview_photo=None; self.pptx_preview_renderer.clear(); self.pptx_file_var.set(self.translator.text("pptx.no_file")); self.update_pptx_preview()
