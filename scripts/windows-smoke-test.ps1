...
try {
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if (Test-Path -LiteralPath $verifyReport) {
            ...
            if ($report.version -ne "1.0.1" -or $report.packaged_ui_evidence.footer_text -ne $expectedFooter -or -not $report.packaged_ui_evidence.footer_visible -or -not $report.packaged_ui_evidence.footer_update_visible -or -not $report.packaged_ui_evidence.footer_update_bound -or $report.packaged_ui_evidence.footer_update_text -eq "update.check") { throw "Packaged footer/version/manual-update verification failed." }
            ...
            if (-not $report.packaged_ui_evidence.update_notification_present -or -not $report.packaged_ui_evidence.update_notification_bound -or -not $report.packaged_ui_evidence.approved_update_handler) { throw "Packaged update-notification functionality is missing." }
