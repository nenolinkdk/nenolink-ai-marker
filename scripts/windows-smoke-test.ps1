...
try {
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if (Test-Path -LiteralPath $verifyReport) {
            ...
            if ($report.version -ne "1.0.1" -or $report.packaged_ui_evidence.footer_text -ne $expectedFooter -or -not $report.packaged_ui_evidence.footer_visible) { throw "Packaged footer/version verification failed." }
